...
- Run against a live X-Plane instance: `uv run python -m sim_pilot --backend xplane --threshold-lat-deg ... --threshold-lon-deg ... [overrides]`. `--bootstrap-from-sim` (alias `--takeoff-from-here`) probes the current aircraft position from X-Plane and uses it as the runway reference.

Requires Python 3.12+. Third-party dependencies are fine — add them to `pyproject.toml` when they earn their keep. Historically the code has leaned on stdlib for several things (hand-rolled YAML parser in `core/config.py`, `urllib` for the OpenAI Responses API, hand-packed UDP in the legacy X-Plane bridge), but that's a starting point, not a rule.
